...
interface RequestData {
  ...
  location: string;
  isVerified?: boolean;
  statusBadge: { label: string; variant: 'pending' | 'action' | 'review' | 'new' | 'accepted' | 'rejected' | 'pending_response' | 'under_review' | 'negotiating'; color?: string };
...
const RequestsPage = () => {
  ...
  const [sentRequestsRecipientFilter, setSentRequestsRecipientFilter] = useState<'landowner' | 'farmer'>('landowner');
  const [selectedRequest, setSelectedRequest] = useState<RequestData | null>(null);
  const [isAgreementsExpanded, setIsAgreementsExpanded] = useState(true);
  const [isFarmerRequestsExpanded, setIsFarmerRequestsExpanded] = useState(true);
  ...
  const journeySteps = selectedRequest?.journeySteps.map(step => {
  ...
  const quickActions = activeTab === 'sent' 
    ? [
        { icon: 'article', label: 'View Land Post', onClick: () => handleQuickAction('view-land'), color: '#3b82f6' },
        { icon: 'message', label: 'Message', onClick: () => handleQuickAction('message'), disabled: selectedRequest?.statusBadge.variant !== 'accepted' },
      ]
    : [
        { icon: 'message', label: 'Message', onClick: () => handleQuickAction('message') },
        { icon: 'videocam', label: 'Schedule Call', onClick: () => handleQuickAction('videocam') },
        { icon: 'summarize', label: 'Notes', onClick: () => handleQuickAction('summarize') },
        { icon: 'block', label: 'Reject', color: '#ef5350', onClick: () => handleQuickAction('reject') },
      ];

  return (
    <DashboardLayout>
      <div style={{background: 'linear-gradient(145deg, #2a2a2a 0%, #1f1f1f 100%)'}} className="text-gray-300 font-sans h-full flex flex-col overflow-hidden transition-colors duration-200">
        <div className="flex-1 overflow-hidden flex flex-col">
        <div className="px-8 py-6 pb-2">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Requests Management</h2>
              <div className="flex items-center mt-1 text-sm text-gray-500 dark:text-gray-400">
                <span className="font-medium text-primary mr-1">12 Actions Needed</span>
                <span>• Categorized farmer inquiries and pending agreements.</span>
              </div>
            </div>
          ...
          <div className="mt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex space-x-2 overflow-x-auto no-scrollbar py-1">
              <FilterChip icon="priority_high" label="Urgent" variant="urgent" />
          ...
          <div className="flex-1 overflow-y-auto p-6 pt-2 space-y-8">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <SectionHeader
                  ...
                  badge={{ label: String(agreementsPendingData.length), variant: 'primary' }}
                  withGradientBar
                />
                <button
                  onClick={() => setIsAgreementsExpanded(!isAgreementsExpanded)}
              ...
              <div className="grid grid-cols-1 gap-4">
                {(agreementsPendingData as RequestData[]).map((agreement) => (
                  <div 
                    key={agreement.id} 
                  ...
                  >
                    <RequestCard
                      ...
                      isSelected={selectedRequest?.id === agreement.id}
                    />
                  </div>
                ))}
              </div>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                />
                <button
                  onClick={() => setIsFarmerRequestsExpanded(!isFarmerRequestsExpanded)}
                  className="text-gray-400 hover:text-white transition-colors"
                >
                  <span className="material-icons">
                    {isFarmerRequestsExpanded ? 'expand_less' : 'expand_more'}
                  </span>
                </button>
              </div>
              {isFarmerRequestsExpanded && (
              <div className="space-y-4">
                {(farmerRequestsData as RequestData[]).map((request) => (
                  <div 
                    key={request.id} 
                  ...
                  >
                    <RequestCard
                      ...
                      primaryAction={{ label: 'Review Request', onClick: () => console.log('Review') }}
            ...
            {activeTab === 'sent' && (
              <div className="space-y-4">
                <div className="flex border-b border-gray-200 dark:border-zinc-800 mb-6">
                  <button
                    onClick={() => setSentRequestsRecipientFilter('landowner')}
                  ...
                  >
                    <span className="material-icons-outlined mr-2 text-lg">landscape</span>
                    Landowners
                    <span
                    ...
                    >
                      {(sentRequestsData as RequestData[]).filter((r) => r.recipientType === 'landowner').length}
                    </span>
                    {sentRequestsRecipientFilter === 'landowner' && (
                      <span
                        className="absolute bottom-0 left-0 right-0 h-0.5"
                        style={{ backgroundColor: '#4ADE80', marginBottom: '-1px' }}
                      ></span>
                    )}
                  </button>
                  <button
                  ...
                  >
                    <span className="material-icons-outlined mr-2 text-lg">agriculture</span>
                    Farmers
                    <span
                      className={`ml-2 text-xs px-2 py-0.5 rounded-full ${
                        sentRequestsRecipientFilter === 'farmer'
                    ...
                    >
                      {(sentRequestsData as RequestData[]).filter((r) => r.recipientType === 'farmer').length}
                    </span>
                    {sentRequestsRecipientFilter === 'farmer' && (
                      <span
                        className="absolute bottom-0 left-0 right-0 h-0.5"
                        style={{ backgroundColor: '#6B8E23', marginBottom: '-1px' }}
                      ></span>
                    )}  
                  </button>
                </div>
                  ...
                  <section className="bg-gray-50 dark:bg-surface-dark/30 rounded-2xl border border-transparent dark:border-gray-700/10 p-4">
                    <div className="flex items-center justify-between mb-4 sticky top-0 py-2 z-10">
                      <h3 className="text-sm uppercase tracking-wide text-gray-500 dark:text-gray-300 font-bold flex items-center gap-2">
                        <span className="material-icons text-primary text-lg">outbox</span>
                        ...
                        <span className="bg-primary/10 dark:bg-primary/20 text-primary-dark dark:text-primary text-xs px-2 py-0.5 rounded-full ml-1 border border-primary/10 dark:border-primary/10">
                          ...
                          {sentRequestsData.filter((r) => r.recipientType === 'landowner').length}
                        </span>
                      </h3>
                    </div>
                    <div className="space-y-4">
                      {(sentRequestsData)
                        .filter((request) => request.recipientType === 'landowner')
                        .map((request) => (
                          <div
                            key={request.id}
                            onClick={() => handleSelectRequest(request as RequestData)}
                          ...
                          >
                            <div className="flex justify-between items-start mb-3">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <h4 className="text-sm font-bold text-gray-900 dark:text-white">{request.farmerName}</h4>
                                  <p className="text-xs text-gray-500 dark:text-gray-400">
                                    {request.recipientType === 'landowner' ? 'Landowner' : 'Farmer'} • {request.location}
                                  </p>
                                </div>
                              </div>
                              <span
                                style={{
                                  background: `${request.statusBadge.color}20` || '#fef3c720',
                                  color: request.statusBadge.color || '#f59e0b',
                                  border: `1px solid ${request.statusBadge.color}10` || '1px solid #f59e0b10',
                                }}
                                className="px-2.5 py-1 rounded text-xs font-semibold flex items-center"
                              >
                                <span className="material-icons text-[14px] mr-1">
                                  {request.statusBadge.variant === 'accepted' ? 'check_circle' : 
                                   request.statusBadge.variant === 'rejected' ? 'block' : 
                                   request.statusBadge.variant === 'under_review' ? 'schedule' : 'pending'}
                                </span>
                                {request.statusBadge.label}
                              </span>
                            </div>
                            <div className="mb-4">
                              {request.tags && request.tags.length > 0 && request.tags[0].label !== 'Sent to Farmer' && request.tags[0].label !== 'Sent to Landowner' && (
                                <div className="p-3 bg-gray-50 dark:bg-zinc-900/50 rounded-lg border border-gray-100/50 dark:border-gray-700/10 mb-3">
                                  <div className="flex items-center gap-2 mb-1">
                                    <span className="material-icons text-gray-400 text-sm">landscape</span>
                                    <h5 className="text-sm font-medium text-gray-800 dark:text-gray-200">{request.tags[0].label}</h5>
                                  </div>
                                  <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1">
                                    Ref #{request.id} • Posted on {request.timestamp}
                                  </p>
                                </div>
                              )}
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-border-dark">
                              <div className="text-xs text-gray-400">Sent: {request.timestamp}</div>
                              <div className="flex gap-2">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                ...
                {sentRequestsRecipientFilter === 'farmer' && (
                  <section className="bg-gray-50 dark:bg-surface-dark/30 rounded-2xl border border-transparent dark:border-gray-700/10 p-4">
                    <div className="flex items-center justify-between mb-4 sticky top-0 py-2 z-10">
                      <h3 className="text-sm uppercase tracking-wide text-gray-500 dark:text-gray-300 font-bold flex items-center gap-2">
                        <span className="material-icons text-blue-500 text-lg">engineering</span>
                        ...
                        <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-xs px-2 py-0.5 rounded-full ml-1 border border-blue-200/50 dark:border-blue-900/10">
                          {(sentRequestsData as any[]).filter((r: any) => r.recipientType === 'farmer').length}
                        </span>
                      </h3>
                    </div>
                    <div className="space-y-4">
                      {(sentRequestsData as any[])
                        .filter((request: any) => request.recipientType === 'farmer')
                        .map((request: any) => (
                          <div
                            key={request.id}
                            onClick={() => handleSelectRequest(request as RequestData)}
                          ...
                          >
                            <div className="flex justify-between items-start mb-3">
                              ...
                              <span
                                style={{
                                  background: `${request.statusBadge.color}20` || '#3b82f620',
                                  color: request.statusBadge.color || '#3b82f6',
                                  border: `1px solid ${request.statusBadge.color}10` || '1px solid #3b82f610',
                                }}
                                className="px-2.5 py-1 rounded text-xs font-semibold flex items-center"
                              >
                                <span className="material-icons text-[14px] mr-1">
                            ...
                            <div className="mb-4">
                              {request.tags && request.tags.length > 0 && request.tags[0].label !== 'Sent to Farmer' && request.tags[0].label !== 'Sent to Landowner' && (
                                <div className="p-3 bg-gray-50 dark:bg-zinc-900/50 rounded-lg border border-gray-100/50 dark:border-gray-700/10 mb-3">
                                  <div className="flex items-center gap-2 mb-1">
                                    <span className="material-icons text-gray-400 text-sm">work_outline</span>
                                    <h5 className="text-sm font-medium text-gray-800 dark:text-gray-200">{request.tags[0].label}</h5>
                                  </div>
                                  <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1">
                                    Ref #{request.id} • Proposal Sent {request.timestamp}
                                  </p>
                                </div>
                              )}
                              <p className="text-sm text-gray-600 dark:text-gray-300">{request.description}</p>
